...
const actions = {
  sendMessage({commit, rootState}, {text, created, sender, conversationId}) {
    ...
    convoRef.update({
      messages: [...state.all[conversationId].messages, {id: uuid.v1(), created, sender, text}]
    })
  ...
  seed({rootState}) {
    ...
    convoRef.add({
      ...
      users: ['mr_a', 'mr_b'],
      messages: [
        {id: uuid.v1(), text: 'Hi there', sender: 'mr_a', created: Date.now()},
        {id: uuid.v1(), text: 'Hi to you too!', sender: 'mr_b', created: Date.now()}
      ]
    });
